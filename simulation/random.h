...
public:
    NormalDistribution(double mean, double variance): mean(mean), variance(variance), distribution(std::normal_distribution<double>(mean, variance)) { }

    // todo: we really need a better approximation for normal_cdf than erf xD
    static double cdf(double value, double mean, double variance) { return 0.5 * (1 + erf((value - mean) / (variance * (M_SQRT1_2)))); }
